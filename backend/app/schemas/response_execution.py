"""Pydantic schemas of the response-execution API.

The client expresses intent only: identity keys (execution_id / approval_id /
compensates_execution_id). The operator identity is resolved server-side from
the Bearer token; the optional ``operator`` field in the request body is
accepted for backwards compatibility but ignored — the authenticated operator
name is the only source of truth. Every execution fact — action, target,
direction, decisions, audit clock — is a server-side construct. extra="forbid"
makes any smuggling attempt (action / target / direction / detail / created_at /
status ...) a 422 at the schema boundary, before the service ever runs.
"""
import uuid
...
"""
    ...
    # operator is optional and ignored when present — the server resolves
    # identity from the Bearer token. Kept optional (not removed) so existing
    # clients that send it still get a 200; the authenticated operator name is
...
approval_id is absent because the service inherits it from the original
execution server-side: a client-supplied approval_id is rejected by
extra="forbid" exactly like any smuggled fact.
"""
    ...
    # Optional and ignored (same rule as ExecuteRequest.operator).
    operator: str | None = Field(default=None, max_length=128)
    comment: str | None = Field(default=None, max_length=512)
...
(succeeded / failed / guard_rejected / compensation_*), straight from the
shared ``derive_execution_state`` rule, never recomputed here.
"""
...
class ExecutionListResponse(BaseModel):
    """GET /executions — paged audit list envelope, mirrors the
IncidentListResponse / EventListResponse shape: the filtered total plus
the requested page, most recent activity first."""
...
#
# Metrics read-model views
#
# Field-for-field mirrors of the three dataclasses in
# services/executions/metrics.py — no second DTO design, no renaming, no
# reshaping: model_validate() serializes the read model field for field.
# Rates keep the None semantics (JSON null on an empty denominator), never
# a fake percentage.
#
...
#
# Observed-health views
#
# Field-for-field mirrors of the three dataclasses in
# services/executions/health.py — read mirrors only, no second model
# (no HealthResponse / HealthItem / AdapterStatus). The verdict word stays
# ``observed_status``: it must not degrade into a boolean ``healthy`` flag,
# which would be misread as a live probe result.
#
