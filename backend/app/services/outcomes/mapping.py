"""Webhook Gate 4 — Semantic Mapping Integration (Phase 3.4.4-D).
...
``map_external_state`` is a deliberately THIN orchestration wrapper. Every
...
``if/elif`` chain, NO copied vocabulary, NO second mapping DTO (spec §3 / §11 /
§16). It only routes the two trusted inputs the mapper needs.

THE TWO INPUTS (spec §8 / §9 / §10):
  - ``observation.adapter`` — the TRUSTED callback identity established by
    ...
    adapter from the validated observation ONLY (spec §9).
  - ``observation.external_state`` — the RAW state, passed through UNMODIFIED.
    ...
    evidences (spec §10). D assumes ``validate_observation`` already ran
    (spec §8) and re-validates NOTHING.

THE SEMANTIC FIREWALLS this gate exists to keep (all enforced in 3.4.3-B and
pinned by tests/test_mapping.py):
  - DISPATCH ≠ OUTCOME (spec §5): the dispatch words ``succeeded`` / ``failed``
  ...
  - NO reconciliation_failed (spec §6): that word is the READ-FAILURE verdict
    (3.4.5), never a state-mapping product. ``StateMapping.__post_init__``
    refuses it STRUCTURALLY, so this gate cannot emit it.
  - unknown ≠ unrecognized (spec §7): ``unknown`` is a RECOGNIZED-but-ambiguous
  ...
  - FAIL-CLOSED ANTI-FABRICATION (spec §4 / §21): only Wazuh has a code-evidenced
...
PURE DOMAIN (spec §14 / §15 / §20): no database, no Session, no ORM, no
...
def map_external_state(observation: NormalizedObservation) -> StateMapping:
    ...
    The Gate 4 integration point (spec §1 / §19): hand the ALREADY-VALIDATED
    ...
    PURE delegation — this function owns NO vocabulary of its own (spec §3):

      - the adapter is ``observation.adapter`` — the TRUSTED Gate-1 identity,
        never a client-supplied value (spec §9);
      ...
      - the outcome word, the raw/normalized state echo and the audit reason are
        all produced by 3.4.3-B, and the result is the frozen ``StateMapping``
        (never an ORM row, spec §11).
    ...
    ``unknown``, NEVER ``reconciliation_failed`` (spec §6 / §7 / §12).
