"""Manual Reconcile Outcome Pipeline.
...
``app/services/manual_reconcile/`` as literally sketched, because that
...
that package without breaking the sealed A1 purity tests, and forbids editing
A1. Design authorizes "具体目录可依现有 repo pattern 微调" as long as Read/Write
...
; a read that SUCCEEDS is handed to
...
with ``UnsupportedAdapterRead`` BEFORE any read —
a CAPABILITY rejection that is NEVER ``reconciliation_failed`` and writes ZERO
facts.

DETERMINISTIC ROW SELECTION (— the crux of this step, DERIVED from the
existing execution service, never invented here). One execution_id maps to
MULTIPLE ``execution_log`` rows, so "which row carries the adapter / the
reference" MUST follow the platform's ordering, not list position:
...
§十一). The read side stays PHYSICALLY isolated from the WRITE side: this
...
``FakeReadAdapter`` in app code / retry / sleep / backoff
...
# ``detail``. ``mock`` is
# ABSENT: it has no external object, so its context carries
# ``external_reference=None`` and A2-C decides UnsupportedAdapterRead
...
# and Wazuh's ``command`` is NOT ``command_id`` — only the
...
class CorrelatedExecutionContext:
    ...
    execution_id's historical dispatch chain refers to.

    Pure facts lifted from ``execution_log`` — carries NO
    ``outcome_status`` (that is derived state, A2-E), NO API credential, NO
    operator token and NO callback token. ``external_reference`` is
    ``None`` ONLY for ``mock``; for
...
def _select_chain(session: Session, execution_id: uuid.UUID) -> list[ExecutionLog]:
    ...
    ``derive_execution_state``'s latest-row DESC). READ-ONLY SELECT.
...
def _extract_adapter(rows: list[ExecutionLog]) -> str:
    ...
    chronological row (— execution-history driven, NEVER the client, the
    path, or the operator). Follows the ``metrics.py::_adapter_of``
    precedent (the requested row).

    A2-C: the adapter is returned AS-IS (any non-empty string). The
    ...
    vocabulary gate is GONE: it pre-empted the registry the spec now
    ...
    as ``MissingExternalReference``, never a guessed adapter."""
...
def _extract_reference(rows: list[ExecutionLog], adapter: str) -> str | None:
    ...
    ``_terminal_outcome_detail`` wrote the executor's response onto, ).

    ``mock`` (no key) -> ``None`` (: recognized, no external object; A2-C
    decides capability). shuffle/wazuh/thehive -> the exact key on the
    terminal row; absent / empty / non-str -> ``MissingExternalReference``
: a chain that never dispatched, failed without a handle, or
    ...
    if key is None:
        return None  # mock — no external object by design
...
def extract_context(
    ...
    """A2-B: correlate then read-only
    extract the adapter + external_reference.

    Two READ-ONLY SELECTs, no writes: ``correlate_execution`` is the
    mandated existence gate; the
    chain rows are then re-selected for extraction because ``CorrelatedExecution``
    exposes neither the rows nor their ``detail``. This is
    the "额外只读 SELECT" / sanctions — NOT a second correlation and
...
def read_external_state(
    ...
    """A2-C: correlation + reference extraction (REUSE ``extract_context``,
    ) -> ``registry.get(adapter)`` -> ``reader.read(request)`` -> the RAW
    ...
    ORDER is load-bearing: ``extract_context`` runs FIRST,
    ...
    adapter) raises ``UnsupportedAdapterRead`` — a rejection, NO
    ...
    NO operator, NO credential, NO client value; this function does
    not even take an ``operator``. C performs NO mapping and NO
    persistence: the raw result is returned untouched.
    """
...
def reconcile_execution(
    ...
    with ``UnsupportedAdapterRead`` BEFORE any
    read; only a test-injected ``FakeReadAdapter`` (/ — NEVER the
    ...
    TWO real exits once a reader is actually invoked (A2-D + A2-E):
      ...
      - the read FAILS at the TRANSPORT layer — a ``ReadTransportError`` or a
        ...
        ``read()`` — is caught HERE and turned into ONE
        ``reconciliation_failed`` Outcome Fact INLINE, then a 200
        ``ManualReconcileResponse``. This is the ONLY path that
    ...
    here, so it still propagates to the router's 404 with ZERO facts (A2-D :
...
.
    ...
    registry instance, never a global mutation.

    ``operator`` is the AUTHENTICATED human recorder identity (``Operator.name``).
    On BOTH paths it becomes the appended fact's ``operator`` (A2-D / A2-E §七
    ...
    and ``AdapterReadRequest`` has no operator / credential field.
    """
    ...
    try:
        ...
        # A2-D : a reader EXISTED and read() was ACTUALLY invoked, then failed in
        ...
        # it is NOT caught here and still rejects 404 / zero-fact. ONE attempt, no
        # retry. read_external_state stays a PURE PROPAGATOR; the conversion lives
        ...
        context = extract_context(session, execution_id)
        # a read failure carries NO trustworthy external timestamp, so the fact time
        ...
        #
        # "然后仍然 validate_observation()": the TIMESTAMP discipline that validator
        ...
        # FUNCTION is NOT called: it validates an ExternalObservation whose
        # contract REQUIRES a present external_state, and a READ FAILURE has NONE (—
        # it never reaches mapping). Fabricating a state to satisfy it would break the
        ...
        # / : a SAFE STATIC classification only — never str(exc), never a callback
        ...
        # / : REUSE webhook.py's persistence vocabulary (the ExecutionOutcomeFact
        ...
        # / : the current state is DERIVED over the whole fact series (latest by
