"""Trusted creation-proof ORCHESTRATION + whitelisted persistence.
...
PLACEMENT REFINEMENT vs Amendment (documented for the reviewer). 's table
...
it REUSES their row-selection helpers (``_select_chain`` / ``_extract_adapter`` /
...
THE CONTROLLED CALL CHAIN. ``reconcile_verified_execution`` is the ONLY caller of the verifier,
...
WHY THE TRUSTED CHANNEL CANNOT USE ``persist_reconcile_outcome``. That
function calls ``map_external_state``, whose thehive vocabulary is EMPTY, so ANY word — including the synthesized ``case_created`` — raises
...
C PROOF-CHANNEL CLOSURE. ``_persist_verified_creation_outcome`` is MODULE-PRIVATE and
...
hand-constructed ``VerifiedCreationEffect`` can NEVER be fed straight to persist. The verify -> authorize -> persist triad CONVERGES into the ONE controlled
...
PERSISTENCE WHITELIST. ``raw_evidence`` / ``raw_response`` are NEVER
...
verification BOOLEANS, the audit-only case number, and the HONEST
...
GATE 5 FAILS CLOSED FOR ALL REAL HISTORY. ``derive_read_correlation_context``
...
historical execution can reach ``confirmed_success`` until the forward-binding Amendment
...
class VerifiedCreationRefused(ContractValidationFailure):
    """The trusted creation-proof verifier REFUSED the read.
...
class UnsealedCreationEffect(RuntimeError):
    """C: ``_persist_verified_creation_outcome`` was handed a ``VerifiedCreationEffect``
NOT minted by ``verify_creation_effect`` (its ``is_sealed()`` is False).

This is the CLOSED boundary the C ruling requires: persist no longer trusts the TYPE
...
def derive_read_correlation_context(
    ...
    """Derive the IMMUTABLE dispatch facts for the strict-correlation half of the proof — the half the ``AdapterReadRequest`` does NOT carry.

READ-ONLY (existence gate + SELECTs; NO write, NO external read, NO mapping). It REUSES
the A2-B row-selection helpers so the adapter + reference are derived IDENTICALLY
...
D: the immutable PRE-DISPATCH BINDING is the
...
CLOSED. NEVER back-filled from the binding's config-declared endpoint.
- ``dispatch_created_at_millis`` is the ``createdAt`` the platform PERSISTED into the
terminal row's ``raw_response`` at dispatch time — the authoritative gate-4 exact-match source; ``None`` if that
...
"""
    ...
    # requested row, rows[-1] the terminal row (the A2-B discipline, reused verbatim).
    ...
    # D: the immutable PRE-DISPATCH BINDING rides in the ``dispatched`` row's
    ...
    # 4 / 5 / 6 refuse exactly as Amendment requires — NEVER back-filled from config.
    ...
    # gate 4 — DISTINCT time semantics. The REAL dispatch START comes from the
...
C PRIVATE + SEALED. MODULE-PRIVATE (``_`` prefix): the controlled service
...
never be fed straight here to write ``confirmed_success``.

Called ONLY after ``verify_creation_effect`` returned the POSITIVE verdict (all six gates
passed). It BYPASSES the shared (empty) vocabulary (see the module docstring):
...
observation time substituted for the historical creation time. The
...
"""
    # C seal gate: refuse an effect NOT minted by the six-gate verifier (fail-closed, ZERO
    ...
    detail = redact_detail(
        {
            ...
            # gate-5 verification BOOLEANS, NEVER the raw instance / tenant VALUES.
...
def reconcile_verified_execution(
    ...
    """The PULL-only trusted creation-proof orchestration.
...
time), the INDEPENDENT read-failure semantics Amendment requires — NEVER
``confirmed_failure``;
...
REAL history gate 5 always refuses, so this
arm is UNREACHABLE until lands — the correct fail-closed state.
...
"""
    ...
    # MissingExternalReference (422). Both are the pipeline's own rejections.
