"""Reconciliation Contract — A (Contract Types + Validation)
+ 3.4.3-B (External State Mapping).

This module is the pure contract layer of the Reconciliation Contract in
...
``ContractValidationFailure`` and NEVER becomes an Outcome Fact;
- trust-domain SHAPE: ``source -> {adapter_callback | human_operator}`` — the shape only, authentication is 3.4.4 / 3.4.5.
3.4.3-B (External State Mapping):
...
vocabulary, returning an immutable ``StateMapping``;
- per-adapter EVIDENCED vocabularies (``ADAPTER_STATE_VOCABULARIES``): only
states that existing adapter code / the design evidence are frozen;
...
REFUSED (no fact), NEVER downgraded to ``unknown``.

WHAT THIS MODULE DELIBERATELY DOES NOT DO:
- NO ``reconciliation_failed``. That word means "the reconcile action ran but
could not READ external state" — a READ-FAILURE path
...
- NO ``execution_id -> existing-chain`` mapping. Here
...
imports ``OUTCOME_STATUSES`` to GUARANTEE every emitted word is a one
(and never ``reconciliation_failed``). The per-adapter external-state words are
NOT imported from the adapters — they are re-here with inline evidence
...
# Bounded clock-skew tolerance for ``observed_at``.
#
...
# default" in , confirmed for the implementation step by
...
# The two identity trust domains. They NEVER merge: a
...
class ContractValidationFailure(Exception):
    """Base class of every Reconciliation Contract rejection.
...
(audit-only), never stored. This is the ``R`` row of : it is
...
class MissingExecutionId(ContractValidationFailure):
    """``execution_id`` absent or not a UUID. STRUCTURAL check
only: whether the id maps to an EXISTING execution chain needs the database and is
therefore 3.4.4 / 3.4.5 — not this pure contract layer."""


class UnknownAdapter(ContractValidationFailure):
    """``adapter`` is not a known adapter identity. Carries the
...
class MissingExternalReference(ContractValidationFailure):
    """``external_reference`` absent / empty. This is a
...
contract does not accept it. This completes the rejection family 3.4.3-A began: ``MissingExternalState`` above is the SHAPE
check, this is the MAPPING check.

THE 铁律: an unrecognized state is NEVER downgraded to ``unknown`` (that
...
class ExternalObservation:
    """The INPUT contract: six required fields
...
class NormalizedObservation:
    """The OUTPUT of a successful contract validation. Immutable (frozen).
...
never be laundered into an outcome word here.
``trust_domain`` records the identity SHAPE so downstream steps
...
def trust_domain_for(source: str) -> str:
    """The identity trust domain a ``source`` belongs to. Pure
lookup over the two-word map. An unknown source is a caller bug —
...
def _normalize_observed_at(observed_at: object, *, now: datetime) -> datetime:
    """Validate + normalize the fact time. PURE.
...
be UTC or local;
- a time later than ``now + MAX_FUTURE_SKEW`` — a future fact
...
does not. Precision is preserved, never truncated;
...
def validate_observation(
    ...
    *,
    ...
    """Validate + normalize one ``ExternalObservation`` against the Reconciliation Contract. PURE: no DB, no session, no
...
REFUSED and produces NO Outcome Fact. Fields are
...
"""
    ...
    # EXISTING chain needs the DB -> 3.4.4 / 3.4.5, .)
    ...
    # 3. external_reference — required, non-empty.
    ...
    # 4. external_state — SHAPE only. Present and non-blank (str)
    # / non-empty (mapping); otherwise preserved RAW. NEVER mapped to an
    # outcome word here (3.4.3-B) and NEVER guessed to unknown.
    ...
    # 5. observed_at — aware, UTC-normalized, bounded future.
    observed_at = _normalize_observed_at(observation.observed_at, now=reference_now)

    # 6. source — one of the two ingress channels; it also
...
# read path is 3.4.5).
#
# THE GOVERNING CONSTRAINT: DO NOT FABRICATE.
# A state word is into a vocabulary ONLY when existing adapter code (or
# the design) evidences it. Where an adapter has no verifiable external
...
# The outcome words ``normalize_external_state`` may produce — the five
# MINUS ``reconciliation_failed``. That fifth word is
...
class AdapterStateVocabulary:
    """ONE adapter's evidenced external-state -> outcome-word mapping. Explicit, auditable, testable — never an if/elif
...
ADAPTER_STATE_VOCABULARIES: dict[str, AdapterStateVocabulary] = {
    "wazuh": AdapterStateVocabulary(
        ...
        # synchronous dispatch response, and is falsified by B0 (agent_status is
        ...
        # command-level effect evidence + an independent Design Freeze;
    ...
    "shuffle": AdapterStateVocabulary(
        adapter="shuffle",
        # GAP: Shuffle is TRIGGER-ONLY — E4 freezes
        ...
        # state word can be without fabrication. The concrete vocabulary
        # lands with the 3.4.5 read path (: "随 3.4.5 adapter read path
    ...
    "thehive": AdapterStateVocabulary(
        adapter="thehive",
        # M2-R SECURITY REMEDIATION (new forward commit; 117ab6b — which added
        # the M2 ``case_created`` word — is untouched, history is read-only).
        # M2 froze EXACTLY ONE synthesized word here, ``case_created``, reasoning
        ...
        # (2-param) is reached by BOTH the LIVE webhook PUSH path
    ...
    "mock": AdapterStateVocabulary(
        adapter="mock",
        # UNSUPPORTED BY DESIGN: Mock has ZERO outbound
...
``mapping_reason`` the auditable why.
"""
    ...
    def __post_init__(self):
        if self.outcome_status not in OUTCOME_STATUSES:
            ...
            # only ever emit a outcome word. The four branches hardcode
...
def _extract_state_word(
    ...
    """Pull the state WORD out of a validated ``external_state``. PURE; returns None when no word can be evidenced. The raw
...
adapter-specifically. PURE: no DB, no HTTP, no ORM, no executor,
...
``UnrecognizedExternalState`` — NO Outcome Fact, NEVER guessed to ``unknown``. Deterministic: the same ``(adapter, external_state)``
...
Wazuh note). M2 then added EXACTLY ONE word — TheHive's synthesized
``case_created`` — but M2-R EMPTIED it again (see the inline TheHive note):
this vocabulary is PATH-AGNOSTIC, so a source-isolated reader-only signal cannot
live in it without being forgeable from the LIVE webhook path, and the 2-param contract cannot express source isolation. The result is that NO adapter
