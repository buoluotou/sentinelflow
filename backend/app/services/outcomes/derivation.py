"""Outcome derivation — a PURE read of External Outcome Facts (Phase 3.4.2).

The Outcome layer is an append-only fact log (design doc
docs/design/phase3.4-execution-outcome-lifecycle.md §4): every row of
...
``app.services.executions.state.derive_execution_state`` (Phase 3.1.3):
...
Vocabulary is imported from ``app.models.execution_outcome`` — one frozen
source, never duplicated here. Dispatch-layer words (``succeeded`` /
...
#: (the fifth of the frozen five), so an execution with no observations
#: yet derives to ``unknown`` — "nothing observed, nothing claimed". It is
#: deliberately NOT None: the dispatch log uses None for "not started"
...
class ForeignOutcomeVocabulary(OutcomeDerivationError):
    """An observation carried a status outside the frozen five-word outcome
...
def derive_outcome_state(observations: Sequence[OutcomeObservation]) -> str:
    ...
    The returned word is guaranteed to belong to the frozen five-word
