"""Manual Reconcile SUCCESS Outcome Persistence.
...
this, and it matches how the read-FAILURE path stamps its fact. Either
...
raw ``AdapterReadResult.raw_evidence`` payload is NOT persisted (only
...
Carries ONLY what ``manual_reconcile.py`` needs to build the ``ManualReconcileResponse`` envelope: the mapped ``outcome_status`` (3.4.3-B),
...
-> NO fact;
4. ``map_external_state`` (3.4.3-B, the ONLY mapping source) -> the ``StateMapping``. Raises ``UnrecognizedExternalState`` (a shuffle / thehive
...
"""
    ...
    # the read-FAILURE path stamps it. validate_observation then
    ...
    # 2. the contract input. source is ALWAYS manual_reconcile (spec §七 /
