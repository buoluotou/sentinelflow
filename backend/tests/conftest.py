...
def client(db_session: Session) -> Generator[TestClient, None, None]:
    ...
    # The durable pre-dispatch store commits on an independent connection,
    ...
    # The reverse seam follows the same rule — the in-memory StaticPool
    # harness shares one connection, so the client fixture keeps the endpoint
    # journeys byte-identical; dedicated file-backed tests drive the real
    # compensation store.
...
#
# Test-only fake adapter for platform success-pipeline proofs
#
# A test-only adapter identity — not a production adapter and not in
# ``registry.ADAPTER_NAMES``. The production external-state vocabularies are
# empty (fail-closed), so no production adapter has an evidenced success word.
# The webhook and manual-reconcile suites need an evidenced success word as the
# vehicle to prove the platform pipeline
# (validate -> map -> confirmed_success -> persist -> 200). Those pipeline
# proofs stay (deleting them would weaken safety) but re-based on an explicit
# test double — never by reopening the Wazuh vocabulary. This fake adapter is
# that double: a separate identity with a test-only evidenced vocabulary,
# injected by ``monkeypatch`` (auto-rollback), so the production four families
# stay empty and every ``set(VOCAB) == set(ADAPTER_NAMES)`` / "all vocabularies
# empty" pin still runs in the un-monkeypatched production state.
...
Leaves ``CALLBACK_ADAPTERS`` untouched — the structural "exactly three
external adapters" pin (test_webhook_authentication.py) stays in production
state — and reuses the declared WAZUH_CALLBACK_TOKEN setting for auth (see
``FAKE_TOKEN_SETTING``).
"""
...
Manual Reconcile path, composing the unit-layer vocabulary.
