"""/ H-1 — Risk <-> Incident transaction atomicity (production debt fix).

The pre-pipeline durably SPLIT at risk recalculation:
...
risk update back. makes the deduplication engine the ONE pipeline
transaction boundary: alert evidence + EventRisk snapshot + automatic Incident
commit or roll back TOGETHER, while the one-case-per-event invariant
...
#
# Normal commit path (rules preserved)
#
def test_normal_pipeline_commits_alert_risk_and_case_together(db_session):
    ...
    # rule: the case COPIES the risk score at creation (snapshot).
...
def test_incident_insert_failure_rolls_back_the_risk_update(db_session, monkeypatch):
    """THE H-1 regression: pre-the risk update was already COMMITTED when
...
def test_recalculate_never_commits_on_its_own(db_session):
    """Direct proof of the fix: pre-``recalculate`` committed internally;
