"""Adapter Callback Authentication tests (Webhook Gate 1).

Locks Gate 1 of the four fixed webhook inbound gates:

External System -> HTTP Webhook
-> Gate 1 Authentication    (this file)
-> Gate 2 Schema
-> Gate 3 Correlation
...
Coverage map:
- Callback config: three *_CALLBACK_TOKEN fields, empty default,
auto-masked, not wired into startup validation, no MOCK_CALLBACK_TOKEN.
- Per-adapter auth: shuffle / wazuh / thehive valid + invalid.
- Cross-adapter isolation: one adapter's token never
authenticates another adapter's channel.
- Mock rejection: mock (and any unknown adapter) is a
404 unsupported channel — never a 401, never a webhook.
- Bearer handling: malformed / non-Bearer / empty /
non-ASCII all collapse to the uniform 401 (never a 500).
- Uniform 401: every failure shape is byte-identical, leaking nothing
about token existence / adapter / length.
- Identity binding: the trusted identity comes from the
server-side route + that adapter's own token, never from client input.
- Operator isolation: no EXECUTION_TOKEN fallback, no
OPERATORS_JSON reuse — the callback trust domain is disjoint.
- Token secrecy: never in a log / response / exception /
repr.
- AST import surface: webhooks.py stays within a bounded callback
allowlist — the Gate-1 auth core {__future__, secrets, fastapi,
app.core.config} plus the wiring the endpoint legitimately needs
(get_db / Session / the Gate-2 body schema / the outcome-fact orchestration
/ the ContractValidationFailure family the router maps to HTTP); it defines
...
Gate 1 scope: this file locks callback authentication. The three HTTP
assertions that depended on a body-less POST reaching a 200 ACK — the
wazuh accepted-ACK placeholder, the all-three-channels placeholder, and the
identity-not-echoed-via-HTTP check — are gone, not
weakened: a real callback must carry a Gate-2 body (a body-less POST is a
...
end-to-end in tests/test_webhook_persistence.py. The Gate-1
rejection surface (uniform 401 / 404, no Outcome Fact) stays here unchanged,
and the Shuffle/TheHive Gate-4 fail-closed behaviour must not
...
# core (__future__, secrets, fastapi, app.core.config) plus the modules the
# callback wiring legitimately needs: the get_db dependency + Session type, the
# Gate-2 body schema / Schema->Contract conversion, the outcome-fact
# orchestration service, and the ContractValidationFailure family the router
# maps to HTTP (that mapping stays in the router; domain exceptions never
# import FastAPI). This is an upper bound — the operator trust domain, the
# executor / dispatch service, outbound adapter IO, derivation, and direct ORM
...
class TestCallbackConfiguration:
    ...
    def test_no_mock_callback_token_field(self):
        # mock is never a callback channel — no token field may exist.
        assert "MOCK_CALLBACK_TOKEN" not in Settings.model_fields

    def test_callback_tokens_are_masked_in_repr(self):
        # values end in TOKEN -> auto-masked; the field NAME stays
    ...
    def test_empty_callback_tokens_do_not_block_startup(self):
        # an unconfigured callback token must NOT prevent app startup —
    ...
    def test_allow_list_is_exactly_the_recognized_adapters(self):
        # /: the callback allow-list == the recognized external adapters
        # (single source of truth), with mock excluded.
    ...
    def test_token_settings_bind_each_adapter_to_its_own_field(self):
        # route adapter -> its OWN server-side token setting name.
...
#
# 7. Bearer handling / malformed authorization
#
...
#
# 8. Uniform 401 — no metadata leakage
#
class TestUniformErrorResponse:
    ...
    def test_every_failure_shape_is_identical_401(self, all_tokens, authz):
        # missing / malformed / non-Bearer / wrong / non-ASCII are ALL
    ...
    def test_unconfigured_and_wrong_token_indistinguishable(self, monkeypatch):
        # "does a token exist?" must never be observable from the response.
        monkeypatch.setattr(settings, "WAZUH_CALLBACK_TOKEN", "")
    ...
    def test_failure_detail_leaks_no_metadata(self):
        # /: the single static detail carries no token, no adapter name,
...
class TestIdentityBinding:
    ...
    def test_identity_ignores_credential_content(self, monkeypatch):
        # a token whose VALUE names other adapters still yields the
...
class TestOperatorIsolation:
    ...
    def test_20_no_operators_json_dependency(self, monkeypatch, no_tokens):
        # /: a valid OPERATOR token must never authenticate a callback.
        monkeypatch.setattr(
            settings,
            "OPERATORS_JSON",
            json.dumps(
...
#
# 12. Endpoint surface — Gate 1 rejection behaviour
#
class TestEndpointSurface:
    ...
    def test_endpoint_writes_no_outcome_fact(self, client, db_session, all_tokens):
        # /: Gate 1 ONLY — no persistence. The append-only
...
#
# 13. AST / import surface — structural security proof
#
...
class TestImportSurface:
    def test_modules_stay_within_the_callback_wiring_allowlist(self):
        # (relaxed for 3.4.4-E): the Gate-1 auth core is ALWAYS present,
        ...
        # 3.4.3 exception family) is in place.
    ...
    def test_no_forbidden_symbol_imported(self):
        # /(relaxed for 3.4.4-E): the callback gate must never pull in
        ...
        # ContractValidationFailure family the router maps to HTTP — keeps
        ...
        # /: operator identity machinery must never be imported.
    ...
    def test_functions_are_exactly_the_sanctioned_three(self):
        # /: only bearer extraction + Gate-1 auth + the stub endpoint.
    ...
    def test_source_never_references_operator_trust_domain(self):
        # source-text scan (safe here — these literals are deliberately
    ...
    def test_source_has_no_persistence_or_execution_surface(self):
        # no DB / executor / schema / mapping construct in the module.
        source = inspect.getsource(webhook_module)
        for forbidden in (
    ...
    def test_authenticate_callback_has_no_body_parameter(self):
        # /: the gate takes ONLY the route adapter + Authorization
